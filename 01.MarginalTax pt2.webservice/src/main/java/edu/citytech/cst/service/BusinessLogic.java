...
public class BusinessLogic {
	...
	if(code.equalsIgnoreCase("QW")) {
		if (yearlySalary >= 0 && yearlySalary < 20000) {
			taxRate=.05f;
		}else if(yearlySalary >= 20000 && yearlySalary < 80000) {
			taxRate=.10f;
		}else if(yearlySalary >= 80000 && yearlySalary < 160000) {
			taxRate=.15f;
		}else if(yearlySalary >= 160000 && yearlySalary < 250000) {
			taxRate=.20f;
		}else if(yearlySalary >= 250000 && yearlySalary < 300000) {
			taxRate=.25f;
	...
	public static String getDescription(String code) {
		String Description = "Not Applicable";
		if (code.equalsIgnoreCase("SI")) {
			Description= "Single Taxable Income";
		} else if(code.equalsIgnoreCase("MFJ")) {
			Description= "Married Filing Jointly";
		} else if(code.equalsIgnoreCase("MFS")) {
			Description= "Married Filing Separately Taxable Income";
		} else if(code.equalsIgnoreCase("HH")) {
			Description= "Head of Household Taxable Income";
		} else if(code.equalsIgnoreCase("QW")) {
			Description= "Qualified Widow(er) Taxable Income";
		}
		return Description;
	}
}
